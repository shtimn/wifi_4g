...
define([ 'jquery', 'knockout', 'config/config', 'service', 'underscore', 'config/menu', "logout" ],
    function ($, ko, config, service, _, menu, logout) {
        ...
        function loginVM() {
            ...
            self.username = ko.observable(data.admin_user);   //fanjj  add username  item
            ...
            self.login = function () {
                ...
                service.login({
                    ...
                    password:self.username()+'\n'+self.password()      //fanjj  add username  item 
                }, function (data) {
                    setTimeout(function () {
                        ...
                        if(config.LOGIN_SECURITY_SUPPORT){
                            self.checkLoginData(function(){
                                if (self.loginCount() == config.MAX_LOGIN_COUNT) {
                                    showAlert("password_error_five_times", function () {          //fanjj  modify "password_error_five_times" displaying  value in  Messages_en.properties 
                                        setFocus();
                                    });
                                    self.startLoginLockInterval();
                                } else {
                                	if(isNaN(self.loginCount())){
                                		showAlert("wifi_must_connect", function() {
                							setFocus();
                						});
                                		return;
                                	}
                                    showAlert({msg: 'password_error_left', params: [config.MAX_LOGIN_COUNT - self.loginCount()]}, function () {   //fanjj  modify 'password_error_left' displaying  value  Messages_en.properties
                                        setFocus();
                                    });
                                }
                            });
                        }else {
                            showAlert("password_error", function () {    //fanjj  modify "password_error_five_times" displaying  value in  Messages_en.properties 
                                setFocus();
                            });
                        }
            ...
            function setFocus(){
                setTimeout(function () {
                	//  var txtUname =$('#txtUname:visible');     //fanjj  add username  item
                    var txtPwd = $('#txtPwd:visible');
                    var txtPIN = $('#txtPIN:visible');
                    var txtPUK = $('#txtPUK:visible');
                   /* if (txtUname.length > 0) {   //fanjj  setFocus for username
                       txtUname.focus();
